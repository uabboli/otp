...
-spec is_key(Key :: term(), Orddict :: orddict()) -> boolean().
...
-spec to_list(Orddict :: orddict(Key, Value)) -> [{Key, Value}].

to_list(Dict) -> Dict.

-spec from_list(List :: [{Key, Value}]) -> Orddict :: orddict(Key, Value).
...
-spec size(Orddict :: orddict()) -> non_neg_integer().

size(D) -> length(D).

-spec is_empty(Orddict :: orddict()) -> boolean().

is_empty([]) -> true;
is_empty([_|_]) -> false.

-spec fetch(Key, Orddict :: orddict(Key, Value)) -> Value.

fetch(Key, [{K,_}|D]) when Key > K -> fetch(Key, D);
fetch(Key, [{K,Value}|_]) when Key == K -> Value.

-spec find(Key :: term(), Orddict :: orddict()) ->
                  {'ok', Value :: term()} | 'error'.
...
-spec fetch_keys(Orddict :: orddict(Key, Value :: term())) -> [Key].

fetch_keys([{Key,_}|Dict]) ->
    [Key|fetch_keys(Dict)];
fetch_keys([]) -> [].

-spec erase(Key :: term(), Orddict1 :: orddict(Key, Value)) ->
      orddict(Key, Value).
...
-spec store(Key, Value, Orddict1 :: orddict(Key1, Value1)) ->
                   Orddict2 :: orddict(Key1 | Key, Value1 | Value).
...
-spec append(Key, Value, Orddict1 :: orddict(Key1, Value1)) ->
                    orddict(Key1 | Key, Value1 | [Value]).
...
-spec append_list(Key, ValList, Orddict1 :: orddict(Key1, Value1)) ->
                     orddict(Key1 | Key, Value1 | ValList) when
      ValList :: [term()].
...
-spec update(Key, Fun, Orddict1 :: orddict(Key, Value1)) ->
                    orddict(Key, Value2) when
...
-spec update(Key, Fun, Initial, Orddict1 :: orddict(Key1, Value1)) ->
                    orddict(Key1 | Key, Value1 | Value2 | Initial) when
      Fun :: fun((Value1) -> Value2).
...
-spec update_counter(Key, Increment, Orddict1 :: orddict(Key1, Value1)) ->
                            Orddict2 :: orddict(Key1 | Key, Value2) when
      Value1 :: number(),
      Value2 :: number(),
      Increment :: number().
...
-spec fold(Fun, Acc0 :: Acc, Orddict :: orddict(Key, Value)) -> AccOut when
...
-spec map(Fun, Orddict1 :: orddict(Key, Value1)) -> orddict(Key, Value2) when
...
-spec filter(Pred, Orddict1 :: orddict(Key, Value)) ->
                    Orddict2 :: orddict(Key, Value) when
...
-spec merge(Fun, Orddict1 :: orddict(Key, Value1),
            Orddict2 :: orddict(Key, Value2)) ->
                   orddict(Key, Value) when
