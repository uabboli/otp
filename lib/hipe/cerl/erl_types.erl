...
-spec t_assign_variables_to_subtype(erl_type(), erl_type()) ->
                            [{erl_type(), erl_type()}].

t_assign_variables_to_subtype(T1, T2) ->
  try assign_vars(T1, T2, [])
  catch
    throw:error ->
      []
  end.

assign_vars(_, ?var(_), _Coll) ->
  erlang:error("Variable in right hand side of assignment");
assign_vars(?any, _, Coll) ->
  Coll;
assign_vars(?var(_)=Var, Type, Coll) ->
  [{Var, Type}|Coll];
assign_vars(_, ?none, Coll) ->
  Coll;
assign_vars(_, ?unit, Coll) ->
  Coll;
assign_vars(?function(Domain1, Range1), ?function(Domain2, Range2), Coll) ->
  Coll1 = assign_vars(Domain1, Domain2, Coll),
  assign_vars(Range1, Range2, Coll1);
assign_vars(?list(Contents1, Termination1, _Size1),
	    ?list(Contents2, Termination2, _Size2), Coll) ->
  Coll1 = assign_vars(Contents1, Contents2, Coll),
  assign_vars(Termination1, Termination2, Coll1);
assign_vars(?product(Types1), ?product(Types2), Coll) ->
  assign_vars_lists(Types1, Types2, Coll);
assign_vars(?tuple(Elements1, Arity, _),
	    ?tuple(Elements2, Arity, _), Coll) when Arity =/= ?any ->
  assign_vars_lists(Elements1, Elements2, Coll);
assign_vars(?tuple_set([{Arity, List1}]),
            ?tuple(Elements2, Arity, _), Coll) when Arity =/= ?any ->
  assign_vars_lists(sup_tuple_elements(List1), Elements2, Coll);
assign_vars(?tuple(Elements2, Arity, _),
            ?tuple_set([{Arity, List1}]), Coll) when Arity =/= ?any ->
  assign_vars_lists(Elements2, sup_tuple_elements(List1), Coll);
assign_vars(?tuple_set(List1), ?tuple_set(List2), Coll) ->
  %% Could probably be improved, but since t_from_form() removes
  %% variables, it should be OK for now.
  try
    assign_vars_lists(lists:append([T || {_Arity, T} <- List1]),
                      lists:append([T || {_Arity, T} <- List2]), Coll)
  catch _:_ ->
      throw(error)
  end;
assign_vars(?opaque(_) = T1, ?opaque(_) = T2, Coll) ->
  assign_vars(t_opaque_structure(T1), t_opaque_structure(T2), Coll);
assign_vars(T1, ?opaque(_) = T2, Coll) ->
  assign_vars(T1, t_opaque_structure(T2), Coll);
assign_vars(?opaque(_) = T1, T2, Coll) ->
  assign_vars(t_opaque_structure(T1), T2, Coll);
assign_vars(?union(U1), T2, Coll) when T2 =/= ?any ->
  ?union(U2) = force_union(T2),
  assign_vars_lists(U1, U2, Coll);
assign_vars(_T1, _T2, Coll) ->
  Coll.

assign_vars_lists([T1|Left1], [T2|Left2], Coll) ->
  assign_vars_lists(Left1, Left2, assign_vars(T1, T2, Coll));
assign_vars_lists([], [], Coll) ->
  Coll.
...
t_map(Fun, ?matchstate(Pres, Slots)) ->
  Fun(?matchstate(t_map(Fun, Pres), t_map(Fun, Slots)));
t_map(Fun, T) ->
  Fun(T).
